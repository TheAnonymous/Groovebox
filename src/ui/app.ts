    ...
    this.root.innerHTML = `
      ...
      <div class="gb-app-shell">
        ...
        <main class="gb-main">
                  ...
                  <div class="gb-bar-row" data-bar-row="${barIndex}">
                    <div class="gb-bar-label">
                      ...
                      <button type="button" class="gb-lock" data-action="toggle-lock" data-bar="${barIndex}" aria-pressed="${state.ui.locks[state.ui.selectedTrack][barIndex]}" title="${state.ui.locks[state.ui.selectedTrack][barIndex] ? "Takt entsperren" : "Takt sperren; Variationen lassen ihn danach unverändert"}">
                        <svg class="bu-icon" aria-hidden="true"><use href="/vendor/braun-ui/icons.svg#lock"></use></svg>
          ...
          <p class="gb-local-note"><svg class="bu-icon" aria-hidden="true"><use href="/vendor/braun-ui/icons.svg#info"></use></svg> Dein Projekt wird ausschließlich in diesem Browserprofil gespeichert. Es gibt in Version 1 keinen Datei- oder Audioexport.</p>
        </main>
        ${this.dialogs(state)}
      </div>
    `;
